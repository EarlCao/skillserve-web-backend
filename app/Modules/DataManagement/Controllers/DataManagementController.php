...
class DataManagementController extends Controller
{
    ...
    #[OA\Delete(path: '/api/data-management/deleted/{type}/{id}', summary: 'Permanently delete a record', description: 'Records of these types are also purged automatically 30 days after deletion.', tags: ['Data Management'], security: [['bearerAuth' => []]], parameters: [new OA\Parameter(name: 'type', in: 'path', required: true, schema: new OA\Schema(type: 'string', enum: ['messages', 'reports', 'reviews'])), new OA\Parameter(name: 'id', in: 'path', required: true, schema: new OA\Schema(type: 'integer'))], responses: [new OA\Response(response: 204, description: 'Record permanently deleted'), new OA\Response(response: 401, description: 'Unauthenticated'), new OA\Response(response: 403, description: 'Unauthorized'), new OA\Response(response: 404, description: 'Record not found'), new OA\Response(response: 422, description: 'Record type cannot be permanently deleted')])]
