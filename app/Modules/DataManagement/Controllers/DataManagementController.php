...
class DataManagementController extends Controller
{
    ...
    #[OA\Delete(path: '/api/data-management/deleted/{type}/{id}', summary: 'Permanently delete a record', description: 'Deleted records are also purged automatically 30 days after deletion. Refused with 409 while related records (e.g. a user\'s bookings) still reference the record.', tags: ['Data Management'], security: [['bearerAuth' => []]], parameters: [new OA\Parameter(name: 'type', in: 'path', required: true, schema: new OA\Schema(type: 'string', enum: ['users', 'services', 'bookings', 'reviews', 'reports', 'messages', 'service_categories', 'service_subcategories'])), new OA\Parameter(name: 'id', in: 'path', required: true, schema: new OA\Schema(type: 'integer'))], responses: [new OA\Response(response: 204, description: 'Record permanently deleted'), new OA\Response(response: 401, description: 'Unauthenticated'), new OA\Response(response: 403, description: 'Unauthorized'), new OA\Response(response: 404, description: 'Record not found'), new OA\Response(response: 409, description: 'Related records still reference this record'), new OA\Response(response: 422, description: 'Record type cannot be permanently deleted')])]
